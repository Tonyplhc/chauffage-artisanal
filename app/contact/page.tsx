      ...
      <PageHeader
        ...
        title={
          <>
            ...
            <em className="not-italic text-bleu">Nous vous rappelons sous 4h ouvrées</em>.
          </>
        }
        intro={
          <>
            Pour un devis, une visite technique, un contrat d&apos;entretien ou une question. Pour l&apos;urgence — composez directement le <strong className="text-bleu font-semibold">numéro 24/7</strong>.
          </>
        }
        aside={
          <HeroAside
      ...
      <section className="py-12 lg:py-20 bg-creme">
        <div className="container grid lg:grid-cols-12 gap-12 lg:gap-16">
          ...
          <div className="lg:col-span-7">
            ...
            <Reveal>
              <h2 className="mt-5 font-display text-3xl lg:text-4xl tracking-tight text-anthra">
            ...
            <form onSubmit={handleSubmit} className="mt-10 grid gap-5">
              ...
              <div className="grid md:grid-cols-2 gap-5">
                ...
                <div>
                  ...
                  <select
                    name="objet"
                    className="w-full bg-white border border-pierre rounded-xl px-4 py-3.5 text-anthra focus:border-bleu focus:outline-none transition-colors"
                  >
                    {SUBJECTS.map((s) => (
              ...
              <div>
                ...
                <textarea
                  name="message"
                  rows={6}
                  className="w-full bg-white border border-pierre rounded-xl px-4 py-3.5 text-anthra focus:border-bleu focus:outline-none transition-colors resize-none"
              ...
              <label className="flex items-start gap-3 text-xs text-taupe">
                <input type="checkbox" className="mt-0.5 accent-bleu" required />
                <span>
              ...
              <motion.button
                ...
                className="group mt-4 inline-flex items-center justify-center gap-2 rounded-full bg-navy text-creme px-7 py-4 text-sm font-medium hover:bg-bleu transition-all hover:-translate-y-0.5 self-start disabled:opacity-60 disabled:pointer-events-none"
              >
                {sent
          ...
          <aside className="lg:col-span-5">
            ...
            <div className="mt-6 rounded-2xl border border-pierre bg-white shadow-soft p-5 lg:p-6">
              <OpeningHoursTable />
            </div>

            <div className="mt-10 p-6 rounded-2xl bg-terracotta/8 border border-terracotta/30">
              <div className="flex items-center gap-2 font-mono text-[10px] uppercase tracking-eyebrow text-terracotta">
              ...
              <p className="mt-3 text-sm text-anthra/90">
              ...
              <a
                ...
                className="mt-4 inline-flex items-center gap-2 text-anthra font-medium hover:text-bleu transition-colors"
              >
        ...
        <div className="container">
          <div className="mb-6 flex items-end justify-between flex-wrap gap-3">
            ...
            <a
              ...
              className="inline-flex items-center gap-2 rounded-full bg-white border border-pierre text-anthra px-4 py-2 text-xs font-mono uppercase tracking-eyebrow hover:border-bleu/40 transition-colors"
            >
...
function InfoCard({
  ...
  const inner = (
    <div className="p-5 lg:p-6 rounded-2xl border border-pierre bg-white hover:border-bleu/40 hover:shadow-lift transition-all flex gap-4 items-start">
      <span className="grid place-items-center h-10 w-10 rounded-full bg-bleu/12 border border-bleu/30 shrink-0">
        <Icon className="h-4 w-4 text-bleu" />
      </span>
      <div>
        <div className="font-mono text-[10px] uppercase tracking-eyebrow text-muted">{title}</div>
        {lines.map((l, i) => (
          <div key={i} className={i === 0 ? "font-medium text-anthra mt-1" : "text-xs text-muted mt-0.5"}>
            {l}
          </div>
        ))}
      </div>
