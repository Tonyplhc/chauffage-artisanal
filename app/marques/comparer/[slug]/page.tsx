        ...
        <div className="container relative">
          ...
          <div className="max-w-3xl">
            ...
            <h1 className="mt-4 font-display text-display-xl tracking-tightest text-balance text-anthra">
              {a.name} <em className="not-italic text-bleu">vs</em> {b.name}
            </h1>
        ...
        <div className="container">
          <div className="max-w-3xl mb-10">
            ...
            <SectionTitle className="mt-4">
              Deux fabricants <em className="not-italic text-bleu">aux ADN distincts</em>.
            </SectionTitle>
          </div>
      ...
      <section className="py-14 lg:py-20 bg-creme">
        <div className="container">
          <div className="max-w-3xl mb-10">
            ...
            <SectionTitle className="mt-4">
              Quand choisir <em className="not-italic text-bleu">l&apos;un ou l&apos;autre</em>.
            </SectionTitle>
          ...
          <div className="mt-8 rounded-2xl border border-pierre bg-white p-6 lg:p-7">
            <div className="font-mono text-[10px] uppercase tracking-eyebrow text-bleu mb-2">
              Point commun
            </div>
            <p className="text-taupe leading-relaxed">{verdict.commonGround}</p>
          </div>
        </div>
      </section>
      ...
      <section className="py-14 lg:py-20 bg-creme border-y border-pierre">
        <div className="container">
          <div className="max-w-3xl mb-10">
            ...
            <SectionTitle className="mt-4">
              Les <em className="not-italic text-bleu">gammes phares</em> côte à côte.
            </SectionTitle>
          </div>
          <div className="grid md:grid-cols-2 gap-6">
      ...
      <section className="py-16 lg:py-20 bg-navy text-creme">
        <div className="container text-center max-w-3xl mx-auto">
          ...
          <h2 className="mt-4 font-display text-display-lg tracking-tightest text-balance">
            {a.name} ou {b.name} ?{" "}
            <em className="not-italic text-sable">Décidons ensemble.</em>
          </h2>
...
function BrandColumn({ brand }: { brand: Brand }) {
  return (
    <div className="rounded-2xl border border-pierre bg-white p-6 lg:p-7">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          ...
          <div className="font-display text-2xl text-anthra tracking-tight">{brand.name}</div>
        </div>
      </div>
      <p className="text-sm text-taupe leading-relaxed mb-4">{brand.positioning}</p>
      ...
      <Link
        ...
        className="inline-flex items-center gap-1.5 text-xs font-mono uppercase tracking-eyebrow text-taupe hover:text-bleu transition-colors"
      >
...
function VerdictCard({
  ...
  return (
    <div className="rounded-2xl border border-bleu/30 bg-white p-6 lg:p-7 shadow-soft">
      ...
      <h3 className="font-display text-xl text-anthra tracking-tight mb-3">{title}</h3>
...
function FlagshipsList({ brand }: { brand: Brand }) {
  return (
    <div className="rounded-2xl border border-pierre bg-white p-6 lg:p-7">
      ...
      <h3 className="font-display text-2xl text-anthra tracking-tight mb-5">{brand.name}</h3>
      <ul className="space-y-3">
        {brand.flagships.map((f) => (
          <li key={f.range} className="flex items-start gap-3 pb-3 border-b border-pierre last:border-0">
            <Check className="h-4 w-4 text-bleu mt-1 shrink-0" />
            <div className="min-w-0">
              <div className="font-medium text-anthra">{f.range}</div>
              <div className="text-[11px] font-mono uppercase tracking-eyebrow text-bleu">
                {f.type}
              </div>
              <p className="mt-1 text-xs text-taupe leading-relaxed">{f.body}</p>
            </div>
          </li>
        ))}
      </ul>
