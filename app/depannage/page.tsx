...
export default function DepannagePage() {
  return (
    <>
      {/* HERO URGENCE */}
      <section className="relative pt-12 lg:pt-16 pb-10 overflow-hidden bg-creme">
        <div className="absolute inset-0 bg-terracotta-glow opacity-100 pointer-events-none" />
        ...
        <div className="container relative">
          <div className="inline-flex items-center gap-3 px-4 py-2 rounded-full bg-terracotta/12 border border-terracotta/40">
            ...
            <span className="font-mono text-[11px] uppercase tracking-eyebrow text-terracotta">
              Astreinte 24/7 · Disponible maintenant
            </span>
          </div>

          <Reveal>
            <h1 className="mt-5 font-display text-display-2xl tracking-tightest max-w-5xl text-balance text-anthra">
              Plus de chaud, plus d'eau —{" "}
              <em className="not-italic text-terracotta">parlez-nous, on s&apos;organise</em>.
            </h1>
          </Reveal>

          <div className="mt-6 grid lg:grid-cols-12 gap-8 lg:gap-12 items-end">
            <Reveal delay={1} className="lg:col-span-7">
              <p className="text-xl text-taupe max-w-2xl text-balance">
                Intervention dépannage selon disponibilité, <strong className="text-terracotta font-semibold">diagnostic prioritaire</strong>, organisation rapide. Conditions d&apos;astreinte précisées au téléphone.
              </p>
              ...
              <div className="mt-8 flex flex-col sm:flex-row items-stretch gap-3">
                <a
                  ...
                  className="group inline-flex items-center justify-center gap-3 rounded-full bg-terracotta text-creme px-8 py-5 text-base font-semibold hover:bg-terracotta/90 transition-all hover:-translate-y-0.5 shadow-card"
                >
          ...
          <Reveal delay={3}>
            <div className="mt-10 grid grid-cols-2 md:grid-cols-4 gap-px bg-sable/60 border border-pierre rounded-2xl overflow-hidden">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-px bg-sable/60 border border-pierre rounded-2xl overflow-hidden">
            {[
              ...
              >
                ...
                <h3 className="mt-4 font-display text-2xl text-anthra tracking-tight">{s.title}</h3>
                <p className="mt-2 text-sm text-taupe leading-relaxed">{s.body}</p>
              </motion.div>
            ))}
          </div>
        </div>
          ...
          <div className="grid md:grid-cols-5 gap-6">
            {[
              ...
              <motion.div
                ...
                className="relative"
              >
                <div className="font-mono text-[11px] uppercase tracking-eyebrow text-terracotta">
                  {s.t}
                </div>
                <div className="mt-4 h-12 w-12 rounded-full bg-terracotta/10 border border-terracotta/30 grid place-items-center">
                  <s.icon className="h-5 w-5 text-terracotta" />
                </div>
                <h3 className="mt-5 font-display text-xl text-anthra tracking-tight">{s.title}</h3>
                <p className="mt-2 text-sm text-taupe leading-relaxed">{s.body}</p>
              </motion.div>
            ))}
          </div>
      ...
      <section className="py-14 lg:py-20 bg-creme border-y border-pierre">
        <div className="container">
          <div className="grid lg:grid-cols-12 gap-12">
            ...
            <div className="lg:col-span-7 grid sm:grid-cols-2 gap-px bg-sable/60 border border-pierre rounded-2xl overflow-hidden">
              {[
                ...
                <div key={p.t} className="bg-white p-6">
                  ...
                  <div className="mt-2 font-display text-2xl text-anthra">{p.p}</div>
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="bg-white p-6">
      ...
      <div className="mt-2 font-display text-3xl text-terracotta">{value}</div>
    </div>
  );
}
