...
export default function GlossairePage() {
  ...
  return (
    <>
        ...
        <div className="container relative">
          <div className="max-w-3xl">
            ...
            <h1 className="mt-4 font-display text-display-xl tracking-tightest text-balance text-ink">
              ...
              <em className="not-italic text-copper">défini précisément</em>.
            </h1>
      ...
      <section className="py-8 bg-linen border-b border-ink/8 sticky top-16 md:top-20 z-20 backdrop-blur-md">
        <div className="container">
          <div className="flex items-center gap-2 flex-wrap">
            <BookOpen className="h-4 w-4 text-copper" />
            <span className="font-mono text-[10px] uppercase tracking-eyebrow text-graphite mr-2">
              Catégories :
            </span>
            {categories.map((c) => (
              <a
                key={c}
                href={`#cat-${c}`}
                className="text-xs px-3 py-1.5 rounded-full bg-white border border-ink/10 text-graphite hover:border-copper hover:text-copper transition-colors"
              >
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {byCategory[c].map((t) => (
                  <article
                    key={t.slug}
                    id={t.slug}
                    className="p-5 lg:p-6 rounded-2xl border border-ink/10 bg-white hover:border-copper/40 transition-colors scroll-mt-32"
                  >
                    ...
                    {t.context && (
                      <div className="mt-3 p-3 rounded-xl bg-cream border border-ink/8">
                        <div className="font-mono text-[10px] uppercase tracking-eyebrow text-copper mb-1">
                          Contexte
                        </div>
                        <p className="text-xs text-graphite leading-relaxed">{t.context}</p>
                      </div>
                    )}
                    {t.source && (
                      <a
                        ...
                        className="mt-3 text-xs text-graphite hover:text-copper inline-flex items-center gap-1.5"
                      >
                        <span className="underline decoration-graphite/30">
                          {t.source.label}
                        </span>
      ...
      <section className="py-14 bg-charcoal text-cream">
        <div className="container text-center max-w-3xl mx-auto">
          <h2 className="font-display text-2xl lg:text-3xl tracking-tight">
            ...
            <em className="not-italic text-copper-200">non traitée ici</em> ?
          </h2>
          ...
          <div className="mt-6 flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link
              href="/contact"
              className="inline-flex items-center gap-2 rounded-full bg-cream text-charcoal px-6 py-3 text-sm font-medium hover:bg-copper hover:text-cream transition-colors group"
            >
              Nous contacter
            ...
            <Link
              href="/devis"
              className="inline-flex items-center gap-2 rounded-full bg-ember/15 border border-ember/40 text-cream px-6 py-3 text-sm font-medium hover:bg-ember/25 transition-colors"
            >
              Demander un devis
            </Link>
          </div>
