    ...
    <>
      ...
      <section className="relative pt-12 lg:pt-16 pb-12 lg:pb-16 bg-creme border-b border-pierre overflow-hidden">
        <div
          ...
          style={{
            backgroundImage:
              "radial-gradient(circle at 25% 35%, rgba(11,87,160,0.30), transparent 55%), radial-gradient(circle at 75% 75%, rgba(11,87,160,0.18), transparent 55%)",
          }}
        />
        <div className="container relative">
          <Link
            href="/marques"
            className="inline-flex items-center gap-2 text-xs font-mono uppercase tracking-eyebrow text-taupe hover:text-bleu transition-colors mb-6"
          >
          ...
          <div className="grid lg:grid-cols-12 gap-10 items-start">
            <div className="lg:col-span-7">
              ...
              <h1 className="mt-4 font-display text-display-xl tracking-tightest text-balance text-anthra">
                {brand.hero.title}{" "}
                <em className="not-italic text-bleu">{brand.hero.titleHighlight}</em>
              </h1>
            ...
            <aside className="lg:col-span-5">
              <div className="rounded-2xl border border-bleu/30 bg-white p-6 lg:p-7 shadow-soft">
                <div className="flex items-center gap-3 mb-5">
                  <span className="grid place-items-center h-10 w-10 rounded-full bg-bleu/12 border border-bleu/30">
                  ...
                  <div>
                    ...
                    <div className="font-display text-xl text-anthra">{brand.name}</div>
                  </div>
                </div>
                <dl className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
                  ...
                  <dd className="text-anthra font-medium">{brand.origin}</dd>
                  <dt className="text-muted font-mono text-[10px] uppercase tracking-eyebrow">
                    Fondée en
                  </dt>
                  <dd className="text-anthra font-medium">{brand.since}</dd>
                  <dt className="text-muted font-mono text-[10px] uppercase tracking-eyebrow">
                    Domaines
                  </dt>
                  <dd className="text-anthra font-medium">
                ...
                <div className="mt-5 pt-5 border-t border-pierre text-xs text-taupe leading-relaxed">
      ...
      <section className="py-14 lg:py-20 bg-creme border-b border-pierre">
        <div className="container max-w-3xl">
          ...
          <SectionTitle className="mt-4">
            Pourquoi {brand.name} <em className="not-italic text-bleu">a sa place</em> dans
            notre catalogue.
          </SectionTitle>
          <p className="mt-6 text-lg text-taupe leading-relaxed">{brand.positioning}</p>
        </div>
      </section>
        ...
        <div className="container">
          <div className="max-w-3xl mb-10">
            ...
            <SectionTitle className="mt-4">
              ...
              <em className="not-italic text-bleu">{brand.name}</em>.
            </SectionTitle>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-px bg-sable/60 border border-pierre rounded-2xl overflow-hidden">
            {brand.whyUs.map((w, i) => (
              <div key={w.title} className="bg-white p-6 lg:p-7">
                ...
                <h3 className="mt-3 font-display text-xl text-anthra tracking-tight">{w.title}</h3>
                <p className="mt-2 text-sm text-taupe leading-relaxed">{w.body}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-14 lg:py-20 bg-creme border-y border-pierre">
        <div className="container">
          <div className="max-w-3xl mb-10">
            ...
            <SectionTitle className="mt-4">
              Les gammes <em className="not-italic text-bleu">phares</em> que nous installons.
            </SectionTitle>
            ...
            {brand.flagships.map((f) => (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <h4 className="font-display text-xl text-anthra tracking-tight">{f.range}</h4>
                  <span className="font-mono text-[10px] uppercase tracking-eyebrow text-bleu bg-bleu/10 border border-bleu/30 px-2 py-1 rounded-full whitespace-nowrap">
                    {f.type}
                  </span>
                </div>
                <p className="mt-3 text-sm text-taupe leading-relaxed">{f.body}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-14 lg:py-20 bg-creme">
        <div className="container grid lg:grid-cols-2 gap-6">
          <div className="rounded-2xl border border-pierre bg-white p-7 lg:p-8">
            <div className="font-mono text-[10px] uppercase tracking-eyebrow text-bleu">
              Garantie
            </div>
            ...
            <p className="mt-4 text-taupe leading-relaxed">{brand.warrantyNote}</p>
          ...
          <div className="rounded-2xl border border-bleu/30 bg-gradient-to-br from-creme to-white p-7 lg:p-8">
            <div className="font-mono text-[10px] uppercase tracking-eyebrow text-bleu">
              Partenariat
            </div>
            <h3 className="mt-3 font-display text-2xl text-anthra tracking-tight">
            ...
            <p className="mt-4 text-taupe leading-relaxed">{brand.partnershipNote}</p>
            <ul className="mt-5 space-y-2 text-sm text-taupe">
              <li className="flex items-start gap-2">
                <Check className="h-4 w-4 text-bleu mt-0.5 shrink-0" />
      ...
      <section className="py-16 lg:py-20 bg-navy text-creme">
        <div className="container text-center max-w-3xl mx-auto">
          ...
          <h2 className="mt-4 font-display text-display-lg tracking-tightest text-balance">
            Une installation {brand.name} <em className="not-italic text-sable">précise</em>,
            de bout en bout.
          </h2>
